/**
 * Credential posture — the first screen, and the reason Fest exists.
 *
...
export function PosturePage({ range }: { range: Range }): React.JSX.Element {
    ...
    <>
      {serverKeyRequests > 0 && (
        <div className="banner">
          {num(serverKeyRequests)} request{serverKeyRequests === 1 ? "" : "s"} in this range ran on
          a <strong>server-held key</strong> rather than a developer's own subscription. That is org
          spend, and it is billed to whoever owns that key — check the Models screen for which model
      ...
      >
        ...
        >
          ...
          <Legend
            ...
            ).map((o) => ({ label: ORIGIN_LABELS[o] ?? o, color: ORIGIN_COLORS[o] ?? "#8b949e" }))}
          />

          <Table head={["Credential origin", "Requests", "People", "Errors", "Tokens", "Cost"]}>
            {origins.map((row) => (
              <tr key={row.credentialOrigin}>
                <td>
                ...
                <td className="num">{num(row.distinctUsers)}</td>
                <td className="num">{row.errors === 0 ? <Muted>0</Muted> : row.errors}</td>
                <td className="num">
                  {tokens(
          ...
          <Table head={["Developer", "5-hour window", "7-day window", "Binding", "Overage", "Observed"]}>
            {(quota.data?.rows ?? []).map((row) => (
              <tr key={row.userId ?? "unattributed"}>
                <td>{userLabel(row.userId, row.email)}</td>
                <td>
                  <span className={`tone-${utilTone(row.fiveHourUtilization)}`}>
                ...
                <td>
                  <span className={`tone-${utilTone(row.sevenDayUtilization)}`}>
                ...
                <td>{row.claim === null ? <Muted>n/a</Muted> : <Pill tone="info">{row.claim}</Pill>}</td>
                <td>
                ...
                <td><Muted>{when(row.observedAt)}</Muted></td>
              </tr>
            ))}
          </Table>
        </QueryState>
